...
import { truncateAddress, formatUsd, formatPercent } from '../foundation/format.js'
...
function buildDevDetail(a: TokenAnalysis): string {
  const parts: string[] = []
  if (a.creatorOpenRatio != null) parts.push(`${(a.creatorOpenRatio * 100).toFixed(0)}% graduation rate`)
...
function buildSmartMoneyDetail(a: TokenAnalysis): string {
  ...
  if (a.smartMoneyVolumeRatio != null) parts.push(`${(a.smartMoneyVolumeRatio * 100).toFixed(0)}% SM volume`)
  if (a.bluechipOwnerPercentage != null) parts.push(`${(a.bluechipOwnerPercentage * 100).toFixed(0)}% bluechip`)
  return parts.join(', ') || 'N/A'
}
    ...
    .action(async (tokenAddress: string, opts: { json?: boolean }) => {
      ...
      try {
        ...
        if (result.holderCount != null || result.priceChange5m != null) {
          console.log()
          const extras: string[] = []
          if (result.holderCount != null) extras.push(`Holders: ${result.holderCount.toLocaleString()}`)
          if (result.priceChange5m != null) extras.push(`5m change: ${formatPercent(result.priceChange5m)}`)
