...
import { formatUsd, formatAge, formatScore } from '../foundation/format.js'
import { scoreColor, brand } from '../foundation/logger.js'
...
function printTable(tokens: TokenAnalysis[], chain: Chain): void {
  ...
  tokens.forEach((t, i) => {
    ...
    const score = t.convictionScore != null
...
async function runScan(chain: Chain, minScore: number | undefined): Promise<void> {
  ...
  if (minScore !== undefined) {
    tokens = tokens.filter(
      (t) => t.convictionScore != null && t.convictionScore >= minScore
    )
  }
...
export function registerScanCommand(program: Command): void {
  program
    ...
    .option('--min-score <n>', 'only show tokens with conviction >= n', (v: string) => parseInt(v, 10))
    .option('--watch', 'continuous mode: poll every 30s and reprint')
    .action(async (opts: { trenches?: boolean; chain?: string; minScore?: number; watch?: boolean }, cmd: Command) => {
      // Chain resolution: --chain on scan > --chain on parent > config default
      let chainRaw: string = opts.chain ?? cmd.parent?.opts().chain ?? loadConfig().defaultChain
      ...
      const minScore: number | undefined = opts.minScore !== undefined && !isNaN(opts.minScore)
        ? opts.minScore
        : undefined
