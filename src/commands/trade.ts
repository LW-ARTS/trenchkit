...
import { loadApiKey, loadConfig, hasPrivateKey } from '../foundation/config.js'
...
export function registerTradeCommand(program: Command): void {
  const trade = program.command('trade').description('Execute swaps and strategy orders (requires private key)')
  ...
  trade
    ...
    .action(async (tokenAddress: string, opts: Record<string, string | boolean>) => {
      ...
      if (!wallet) {
        console.error(brand.error('Missing wallet address. Pass --wallet or set walletAddress in config.json'))
        process.exitCode = 1; return
      }
      ...
      const tpSl = parseTpSl(opts as { tp?: string; sl?: string; trailTp?: string; trailSl?: string })
  ...
  trade
    ...
    .action(async (tokenAddress: string, opts: Record<string, string | boolean>) => {
      ...
      try {
        ...
        const position = holdings.find((h) => h.token_address.toLowerCase() === tokenAddress.toLowerCase())
        if (!position || position.balance <= 0) {
          console.error(brand.error(`No ${truncateAddress(tokenAddress)} balance found in ${truncateAddress(wallet)}`))
          process.exitCode = 1; return
        }
  ...
  trade
    ...
    .action(async (orderId: string) => {
      ...
      try {
        ...
        if (status.tx_hash) console.log(`  Tx: ${chalk.dim(getExplorerTxUrl(chain, status.tx_hash))}`)
    ...
    .action(async (opts: { wallet?: string }) => {
      ...
      try {
        ...
        for (const o of orders) {
          const trigger = o.drawdown_rate != null
