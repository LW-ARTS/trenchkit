...
export function createTokenApi(ctx: ApiContext): TokenApi {
  return {
    ...
    getSecurity(chain: Chain, address: string): Promise<GmgnTokenSecurity> {
      return ctx.request<GmgnTokenSecurity>(
        'GET',
        `/api/v1/token/${chain}/${address}/security`,
        { weight: 1 }
      );
    },
    ...
    getTopHolders(
      ...
      return ctx.request<GmgnHolder[]>(
        'GET',
        `/api/v1/token/${chain}/${address}/top_holders`,
        { params, weight: 5 }
      );
    },
    ...
    getTopTraders(
      ...
      if (options.limit !== undefined) params['limit'] = String(options.limit);
      if (options.cursor !== undefined) params['cursor'] = options.cursor;
      return ctx.request<GmgnRankItem[]>(
        'GET',
        `/api/v1/token/${chain}/${address}/top_traders`,
        { params, weight: 5 }
      );
    },
  };
