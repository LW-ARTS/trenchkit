...
import type { GmgnWalletHolding, GmgnWalletStats } from "../foundation/api-types.js";
...
function scorePositionSizing(stats: GmgnWalletStats, holdings: GmgnWalletHolding[]): number {
  ...
  if (ratio <= 0.1) return 60 + (ratio / 0.1) * 40;
  if (ratio <= 0.2) return 100 - ((ratio - 0.1) / 0.1) * 40;
  // Over-concentrated (>20% per position)
  return Math.max(0, 60 - ((ratio - 0.2) / 0.2) * 60);
}
