...
describe("executeTrade", () => {
  ...
  it("surfaces no-retry guidance on AUTH_TIMESTAMP_EXPIRED, never retries POST", async () => {
    ...
    await expect(executeTrade(client, buyIntent(), baseConfig, { yes: true })).rejects.toThrow(
      /DO NOT retry/i,
    );
...
describe("pollOrder", () => {
  ...
  it("times out if never confirms", async () => {
    ...
    await expect(pollOrder(client, "sol", "o1", { intervalMs: 1, maxMs: 5 })).rejects.toThrow(
      /polling timed out/i,
    );
  });
});
